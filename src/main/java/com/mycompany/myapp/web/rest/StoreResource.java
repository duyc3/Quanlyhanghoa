...
public class StoreResource {
    ...
    public ResponseEntity<Store> updateStore(@PathVariable(value = "id", required = false) final Long id, @Valid @RequestBody Store store)
        ...
        Optional<Store> result = storeRepository
            ...
            .map(existingStore -> {
                if (store.getNameStore() != null) {
                    existingStore.setNameStore(store.getNameStore());
                }
                if (store.getAddressStore() != null) {
                    existingStore.setAddressStore(store.getAddressStore());
                }
                if (store.getCreateDate() != null) {
                    existingStore.setCreateDate(store.getCreateDate());
                }
                if (store.getCreateBy() != null) {
                    existingStore.setCreateBy(store.getCreateBy());
                }
